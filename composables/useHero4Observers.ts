...
export const useHero4Observers = (
  ...
  const setupCarouselObserver = () => {
    if (typeof IntersectionObserver === 'undefined' || !refs.section.value) return
      ...
      (entries) => {
        entries.forEach((entry) => {
          ...
          isCarouselVisible.value =
            entry.isIntersecting && entry.intersectionRatio > INTERSECTION_THRESHOLD
    ...
    observer.observe(refs.section.value)
  }
  ...
  const setupBenefitsObserver = () => {
    ...
    if (refs.benefitsMobile.value) benefitsObserver.observe(refs.benefitsMobile.value)
  }
  ...
  onBeforeUnmount(() => {
    ...
    if (benefitsObserver) {
      ...
      if (refs.benefitsMobile.value) benefitsObserver.unobserve(refs.benefitsMobile.value)
